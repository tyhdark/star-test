...
def setup_update_region():
    logger.info("setup_update_region_data")
    region = test_region.TestRegion()
    region_admin_addr, region_id = region.test_create_region()

    # 更新一个不存在的region_name
    _, region_name = handle_name.create_region_id_and_name()

    update_region_data = [
        dict(region_id=region_id, from_addr=region_admin_addr, fees="1", region_name=region_name),
    ...
    yield region_admin_addr, region_id, update_region_data
