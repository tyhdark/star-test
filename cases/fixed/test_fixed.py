...
with open(current_path + '/test_fixed.yml', 'r') as file:
...
class TestFixed:
    ...
    def test_fixture(self, creat_one_kyc_region):
        user_addr, amount, region_id = creat_one_kyc_region
    ...
    def test_fixed_success_amount_and_mouth(self, test_fixed, creat_one_kyc_region):
        """
    ...
    # 测试最小金额
    @pytest.mark.parametrize("test_fixed", test_data)
    def test_fixed_min_or_max_amount(self, test_fixed, creat_one_kyc_region):
        """
        ...
        logger.info(f"result========{str(http_result)}")

        assert "amount is less than 0.01ME" in str(http_result) or "fixed deposit amount error" in str(http_result)
        pass

    # 测试最大月份
    @pytest.mark.parametrize("test_fixed", [2, 4, 5, 7, 8, 10, 11, 13, 14, 15, 16, 17, 19, 20, 21])
    def test_fixed_error_mouth(self, test_fixed, creat_one_kyc_region):
        user_addr, start_balance, region_id = creat_one_kyc_region
        # user_addr, region_id = self.test_get_kyc_info()
        mouth = test_fixed
        # 发起定期委托
    ...
    # 测试非KYC用户发起定期委托
    def test_fixed_no_kyc(self, creat_one_nokyc):
        """
        ...
        amount = "1"
        mouth = "1"
        # logger.info(f"user_balance={user_balances}")
        # 用户发起定期委托
        result = Tx.Staking.deposit_fixed(from_addr=user_addr, amount=amount, month=mouth)
        # logger.info(f"result={result}")
        time.sleep(5)
        http_result = HttpQuery.Tx.query_tx(tx_hash=result['txhash'])
        user_balances_end = HttpQuery.Bank.query_balances(addr=user_addr)
        # logger.info(f"result={http_result}")
        assert "only kyc user can do fixed deposit" in str(http_result)
        assert user_balances_end == int(user_balances) - 100
...
class TestFixedWithdraw:
    ...
    # 正常提取定期，计算收益
    def test_withdraw_expire(self, creat_one_kyc_region):
        """
        发起一笔定期，等待60秒到期后提取收益
        """
        # 用户发起定期委托
        ...
        logger.info(f"fixed_info = {fixed_info}")
        ...
        logger.info(f"end_balance={end_balance}")
        assert end_balance == start_balance - 100 - 100 + reward
    ...
    def test_withdraw_no_expire(self, creat_one_kyc_region):
        ...
        # 先发起一笔定期
        Tx.Staking.deposit_fixed(from_addr=user_addr, amount=fixed_amount, month=month)
        time.sleep(7)
        # 查出用户定期id 拿到用户的委托id
        fixed_info = HttpQuery.Staking.fixed_deposit(addr=user_addr)
    ...
    # 输入已经提取过的id
    def test_deposit_id_done(self, creat_one_kyc_region):
        ...
        http_result = HttpQuery.Tx.query_tx(tx_hash=repeat_withdraw_info['txhash'])
        time.sleep(7)
        # 获取用户余额
        end_balance = HttpQuery.Bank.query_balances(addr=user_addr)
        user_fixed_list_end = HttpQuery.Staking.fixed_deposit(addr=user_addr)
        all_fixed_list_end = HttpQuery.Staking.fixed_deposit()
        # 断言响应
        assert "fixed deposit not found" in str(http_result)
        # 断言用户余额
    ...
    # 换一个人提取定期
    def test_deposit_withdraw_payee_error(self, creat_one_kyc_region, creat_one_kyc):
        """A发起定期，B去提取A的定期"""
        user_addr_a, user_a_balances, region_id = creat_one_kyc_region
        user_addr_b, user_b_balances = creat_one_kyc
        ...
        http_result = HttpQuery.Tx.query_tx(tx_hash=withdraw_info['txhash'])
        # print(withdraw_info)
        time.sleep(7)
        # 获取用户余额
        user_fixed_list_end = HttpQuery.Staking.fixed_deposit(addr=user_addr_a)
        all_fixed_list_end = HttpQuery.Staking.fixed_deposit()
        assert "only depositor can withdraw (fixed deposit payee error): do fixed withdraw error" in str(http_result)
        # 断言个人和区域委托列表没有任何变化
        assert len(user_fixed_list_end) == len(user_fixed_list_start) + 1
        assert len(all_fixed_list_end) == len(all_fixed_list_start) + 1
        # 提取定期打扫数据
